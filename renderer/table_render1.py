...
class DoubleTableRender(Render, metaclass=ABCMeta):
    ...
    # 填充答题区域
    def answer_space_handler(self, answer_symbol, table, i, j, question_form):
        """
        :param self:
        :param answer_symbol:
        :param table: 承载数据的表格
        :param i: answer_space区域的行
        :param j: answer_space区域的列
        :return:
        """
        # 处理单词的answer_space
    ...
    # 句子题处理器：如果句子提示词没有{}，则根据答案子串自动添加{}，以便后续同一处理
    @staticmethod
    def sentence_handler(description, answer):
    ...
    # 句子题处理器：把description中{}包裹的内容替换为*
    @staticmethod
    def sentence_star_handler(description):
    ...
    # 句子题渲染器
    def sentence_render(self, merged_cell, item_node, question_form):
    ...
    def render(self):
        ...
        for i in range(rows_num):
            for j in range(columns_num):
                # 如果是奇数行 or 正在添加句子题，添加题目
                if (i + 1) % 2 == 1 or is_sentence == True:
                    # 如果data为空，跳出循环
                    if data.empty():
                        break
                    item_node_queue = data.get()  # 获取题目node元组，第一个元素是题目node
                    question_form = item_node_queue[1]  # 获取题目form：word sentence
                    item_order = item_node_queue[2]  # 获取题目顺序
                    item_node = item_node_queue[0]  # 获取题目node
                    if question_form == 'word':  # 如果是词语形式的题目
                        ...
                        if is_sentence is False and j > 0:  # 第一道句子题且正在填充单词行，则应该立即跳转到下一行
                            is_sentence = True  # 句子题开始的标志
                            shim = 2  # 写句子要跳过正在处理的单词行和answer_space
                            # 处理单词的answer_space
                            for k in range(j):
                                self.answer_space_handler(answer_symbol, table, i + 1, k, question_form)
                        description = item_node.item_description  # 题目主提示词
                        answer = item_node.answer

                        # 检查description中有无{}，如果没有，则查找answer子串，忽略大小写，并用{}包裹
                        description = self.sentence_handler(description, answer)

                        # 检查description中有无{}，如果有，则循环替换
                        description = self.sentence_star_handler(description)
                        ...
                        # 渲染句子题
                        self.sentence_render(merged_cell, item_node, question_form)
                    else:
                    ...
                    if (math.floor(i / 2 + 1) - 1) * columns_num + j + 1 > self.get_data().qsize():
                        break
                    # 处理答题区域
                    self.answer_space_handler(answer_symbol, table, i, j, question_form)
